...
class AppMaintenance(unittest.TestCase):
    def setUp(self):
    ...
    def test_app_maintenance(self):
        ...
        driver.find_element_by_link_text("Add/Edit Maintenance").click()
        driver.find_element_by_id("id_notes").click()
        driver.find_element_by_id("id_notes").clear()
        driver.find_element_by_id("id_notes").send_keys("Just for testing.")
        driver.find_element_by_xpath("(.//*[normalize-space(text()) and normalize-space(.)='*'])[1]/following::button[1]").click()
        self.assertEqual("Maintenance notes modified Successfully!", driver.find_element_by_xpath(u"(.//*[normalize-space(text()) and normalize-space(.)='← Go back to App Page'])[1]/following::h3[1]").text)
